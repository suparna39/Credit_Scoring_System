...
export const CONTRACT_ADDRESS =
  "CB5UXQTWZQP7QFUK223F6WXTESE5MGJGN4RGXKQFIW3YQHYHZQUWN3ZK";
...
export function toScValI32(value: number): xdr.ScVal {
...
/**
 * Initialize a user's credit profile.
 * Calls: init_user(user: Address)
 * Sets initial score to 500 with 0 transactions.
 */
export async function initUser(caller: string, user: string) {
  return callContract(
    "init_user",
    [toScValAddress(user)],
    caller,
 ...
 * Calls: update_score(user: Address, delta: i32)
 * Delta can be positive (increase) or negative (decrease).
 * Score clamped between 300-900.
 */
export async function updateScore(
  ...
  return callContract(
    "update_score",
    [toScValAddress(user), toScValI32(delta)],
    caller,
    true
  );
}
...
/**
 * Get a user's credit score (read-only).
 * Calls: get_score(user: Address) -> u32
 * Returns: credit score number
 */
export async function getScore(user: string, caller?: string) {
  return readContract(
    "get_score",
    [toScValAddress(user)],
    caller
...
/**
 * Get a user's full credit profile (read-only).
 * Calls: get_profile(user: Address) -> CreditData
 * Returns: { score: number, transactions: number }
 */
